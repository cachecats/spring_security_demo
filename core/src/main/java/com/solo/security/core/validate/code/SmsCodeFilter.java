...
public class SmsCodeFilter extends OncePerRequestFilter implements InitializingBean {
  ...
  private final String SMS_CODE_SESSION_KEY = ValidateCodeProcessor.SESSION_KEY_PREFIX + "SMS";
  ...
  private void validate(ServletWebRequest servletWebRequest) throws ServletRequestBindingException {
    ...
    ValidateCode smsCode = (ValidateCode) sessionStrategy
        .getAttribute(servletWebRequest, SMS_CODE_SESSION_KEY);
    //获取请求中用户提交的验证码表单
    String codeInRequest = ServletRequestUtils
        .getStringParameter(servletWebRequest.getRequest(), "smsCode");

    log.info("codeInRequest: {}", codeInRequest);
    if (smsCode != null){
      log.info("smsCode: {}", smsCode.getCode());
    }
    ...
    if (smsCode.isExpired()) {
      //将验证码从 session 中移除
      sessionStrategy.removeAttribute(servletWebRequest, SMS_CODE_SESSION_KEY);
